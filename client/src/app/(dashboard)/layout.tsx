...
const HR_MANAGER_LINKS = [
  { href: "/hr-manager", label: "Dashboard", icon: LayoutDashboard },
  { href: "/hr-manager/job-postings", label: "Job Postings", icon: Target },
];

// SIDEBAR_LINKS will be computed inside the component when pathname is available
...
export default function DashboardLayout({
  ...
  // choose links based on current path
  const SIDEBAR_LINKS = pathname.startsWith("/hr-manager")
    ? HR_MANAGER_LINKS
    : HR_ADMIN_LINKS;
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC]">
      {/* Sidebar */}
      <aside className="w-64 bg-white border-r border-[#E2E8F0] hidden md:flex md:flex-col">
        <div className="p-6">
          <div className="flex items-center gap-3">
            <div className="bg-blue-700 bg-linear-to-br from-blue-600 to-indigo-700 rounded-lg p-2 shadow-sm border border-blue-500">
              <Users className="text-white w-6 h-6" />
            </div>
            <span className="text-2xl font-black text-black tracking-tight">
              TalentHQ<span className="text-blue-600">.</span>
            </span>
          </div>
        </div>
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          {SIDEBAR_LINKS.map((link) => {
            const isActive =
              pathname === link.href ||
              (pathname.startsWith(link.href) && link.href !== "/hr-admin");
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 ${isActive ? "text-blue-700" : "text-slate-400"}`}
                />
                {link.label}
              </Link>
            );
        ...
        <div className="p-4 border-t border-slate-200">
          <div className="flex items-center gap-3 px-3 py-2 cursor-pointer hover:bg-slate-50 rounded-lg transition-colors">
            ...
            />
            <div>
              <p className="text-sm font-bold text-black">Aanya Gupta</p>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="hidden md:flex flex-col">
              <h2 className="text-lg font-bold text-black tracking-tight">
                ...
                {pathname === "/hr-admin" || pathname === "/hr-manager"
                  ? "Dashboard Overview"
                  : pathname.split("/").pop()?.toUpperCase()}
              </h2>
              <p className="text-xs text-black font-medium tracking-wide">
                TalentHQ Management Portal
              </p>
            </div>
          </div>
