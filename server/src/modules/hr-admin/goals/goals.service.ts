...
import { GoalType, GoalStatus } from "../../../generated/prisma/enums.js";
...
export class GoalsService {
  ...
  static async getSummary(employeeId: string) {
    ...
    const calculateRate = (type: string) => {
      ...
      const completed = typeGoals.filter(
        (g) => g.status === "COMPLETED",
      ).length;
      return typeGoals.length
        ? Math.round((completed / typeGoals.length) * 100)
        : 0;
    };
